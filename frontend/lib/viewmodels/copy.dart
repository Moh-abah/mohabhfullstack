...
class StoresState with ChangeNotifier {
  ...
  //   } catch (e) {
  //     _logger.e('لم يتم العثور على متجر بالمعرف: $id');
  //     return null;
  //   }
  // }

  // double calculateDistance(Store store, LatLng? userLocation) {
  ...
  //     );
  //   } catch (e) {
  //     _logger.e('خطأ في حساب المسافة للمتجر ${store.name_store}: $e');
...
class StoreListProvider with ChangeNotifier {
  ...
  Future<void> onStoreTapped(BuildContext context, Store store) async {
    try {
      // حفظ موقع الكاميرا الحالي قبل فتح التفاصيل
      await saveCurrentCameraPosition();

      // تعيين المتجر المحدد
      storesState.setSelectedStoreId(store.id);

      // تحديث بيانات المتجر أولاً
      await storesState.updateStoreRating(store.id);

      // جلب المراجعات بشكل منفصل
      await reviewsState.fetchStoreReviews(store.id).catchError((error) {
        uiState.showErrorSnackBar(context, 'فشل في تحميل المراجعات');
      });
